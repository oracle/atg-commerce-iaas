...
def listAccounts(endpoint, resourcename, cookie):
    basepath = '/account/'
    params = None
    data = None
...
def main(argv):
    ...
    try:
        ...
        if moduleArgs['cookie'] is not None:
            jsonObj = listAccounts(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'])
            printJSON(jsonObj)
        else:
