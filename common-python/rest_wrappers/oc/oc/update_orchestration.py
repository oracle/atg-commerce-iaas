...
__module__ = "update-orchestration"
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
def readModuleArgs(opts, args):
    ...
    for opt, arg in opts:
        ...
        elif opt in ("-o", "--orchestration"):
            moduleArgs['orchestration'] = json.loads(arg)
...
def main(argv):
    ...
    options = 'e:u:p:P:R:C:o:j:'
    longOptions = ['endpoint=', 'user=', 'password=', 'pwdfile=', 'resourcename=', 'cookie=', 'orchestration=',
                   'jsonfile=']
        ...
        if moduleArgs['cookie'] is not None:
            if moduleArgs['jsonfile'] is not None:
                with open(moduleArgs['jsonfile'], 'r') as f:
                    moduleArgs['orchestration'] = json.load(f)
            jsonObj = updateOrchestration(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'],
                                          moduleArgs['orchestration'])
            printJSON(jsonObj)
        else:
