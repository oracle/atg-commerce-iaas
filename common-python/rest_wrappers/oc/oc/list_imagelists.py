...
def listImagelists(endpoint, resourcename, cookie):
    basepath = '/imagelist/'
    params = None
    data = None
...
def main(argv):
    ...
    try:
        ...
        if moduleArgs['cookie'] is not None:
            jsonObj = listImagelists(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'])
            printJSON(jsonObj)
        else:
