...
def listInstances(endpoint, resourcename, cookie):
    basepath = '/instance/'
    params = None
    data = None
...
def main(argv):
    ...
    try:
        ...
        if moduleArgs['cookie'] is not None:
            jsonObj = listInstances(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'])
            printJSON(jsonObj)
        else:
