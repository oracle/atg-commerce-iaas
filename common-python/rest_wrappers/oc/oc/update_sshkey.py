...
def updateSshKey(endpoint, resourcename, cookie, name, key, enabled):
    basepath = '/sshkey/'
    params = None
...
def main(argv):
    ...
    options = 'e:u:p:P:R:C:k:K:n:d'
    longOptions = ['endpoint=', 'user=', 'password=', 'pwdfile=', 'resourcename=', 'cookie=', 'key=',
                   'keyfile=', 'name=', 'disabled']
    ...
    try:
        ...
        if moduleArgs['cookie'] is not None:
            if moduleArgs['keyfile'] is not None:
                with open(moduleArgs['keyfile'], 'r') as f:
                    moduleArgs['key'] = f.read().rstrip('\n')
            jsonObj = updateSshKey(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'],
                                   moduleArgs['name'], moduleArgs['key'], moduleArgs['enabled'])
            printJSON(jsonObj)
        else:
