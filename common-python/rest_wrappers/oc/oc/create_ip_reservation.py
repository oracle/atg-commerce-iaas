...
__module__ = "create-ip-reservation"
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
def createIPReservation(endpoint, resourcename, cookie, name=None, parentpool='/oracle/public/ippool', permanent=True, **kwargs):
    basepath = '/ip/reservation/'
    params = None
    data = {"parentpool": parentpool, "permanent": permanent}
    if name is not None:
        data['name'] = str(name)
...
def main(argv):
    ...
    try:
        ...
        if moduleArgs['cookie'] is not None:
            jsonObj = createIPReservation(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'],
                                          moduleArgs['name'], moduleArgs['parentpool'], moduleArgs['account'],
                                          moduleArgs['permenant'])
            printJSON(jsonObj)
        else:
