...
def deleteImagelist(endpoint, resourcename, cookie):
    basepath = '/imagelist/'
    params = None
    data = None
...
def main(argv):
    ...
    try:
        ...
        if moduleArgs['cookie'] is not None:
            jsonObj = deleteImagelist(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'])
            printJSON(jsonObj)
        else:
