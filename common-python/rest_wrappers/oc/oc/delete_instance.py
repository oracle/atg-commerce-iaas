...
def deleteInstance(endpoint, resourcename, cookie):
    basepath = '/instance/'
    params = None
    data = None
...
def main(argv):
    ...
    try:
        ...
        if moduleArgs['cookie'] is not None:
            jsonObj = deleteInstance(moduleArgs['endpoint'], moduleArgs['resourcename'], moduleArgs['cookie'])
            printJSON(jsonObj)
        else:
